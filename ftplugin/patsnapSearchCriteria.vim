augroup filetype_patsnapSearchCriteria
    autocmd!
...
function! s:PrettyLine(line)
    let l:line = substitute(a:line, '\c\([a-z0-9_]\+\)\s*:\s*', '\L\1:', 'g')
    let l:line = substitute(l:line, '\c\S\zs\s*\<\(and\|or\|gand\|to\)\>\s*\ze\S', ' \L\1 ', 'g')
...
function! s:CopyOneLine() range
    ...
    let @+ = trim(l:string)
    echom "Oneline-patsnapSearchCriteria has been copied to clipboard!"
    unlet l:lines
endfunction
